"""
my_manim_lib.mobjects — custom Mobject classes
==============================================

সতর্কবার্তা / Status notes (download করলেই পড়ে নাও)
-------------------------------------------------
সব ক্লাস কোডে আছে। নিচের নোট দেখে কোনটা কখন ব্যবহার করবে ঠিক করো।

RECOMMENDED
  • GlowLine, GlowDot, Glow     — layered glow (VMobject based)
  • BrightGlowDot, MultiBrightGlow — image-based bright glow
  • SmoothCross                 — soft X mark over a mobject
  • SplitTex, SplitText         — word/letter split helpers
  • ScreenBlur                  — full-screen blur (normal Scene + MovingCamera)
  • TrueGaussianBlur2           — soft Gaussian glow from a shape
                                  (sigma + intensity দিয়ে মাত্রা কন্ট্রোল)

NOTES / LIMITATIONS
  • ScreenBlur
      - পুরো ফ্রেমের snapshot ব্লার করে
      - normal ``Scene`` এবং ``MovingCameraScene`` দুটোতেই কাজ করে
  • TrueGaussianBlur2
      - true filled raster blur না; curve-sample + Gaussian glow
      - মাত্রা বোঝার জন্য: sigma (ছড়ানো), intensity (জোর)
      - উদাহরণ: sigma=4 subtle, sigma=12 medium, sigma=20 soft + intensity=1.4 strong
  • Glow* vs BrightGlow*
      - Glow* = অনেকগুলো semi-transparent stroke/fill layer (vector)
      - BrightGlow* = pixel image glow (matplotlib colormap)

কেন কিছু জিনিস “মুছে” না রেখে নোট রাখা হয়েছে?
  animations.py-তে legacy duplicates একই কারণে রাখা — পুরনো স্ক্রিপ্ট না ভাঙুক,
  কিন্তু নতুন কোডে recommended API ব্যবহার করো।
"""
...
class ScreenBlur(ImageMobject):
    def __init__(self, scene, sigma=15, **kwargs):
        """
        Full-screen Gaussian blur of the current frame.

        Works with both normal ``Scene`` (Cairo Camera) and
        ``MovingCameraScene`` (camera has a ``frame`` mobject).
        ...
        scene : Scene
            Current scene (self).
        sigma : float
            Blur strength (scaled with render resolution).
        """
        # Force the renderer to bake the current frame
        scene.renderer.update_frame(scene)
        screen_pixels = scene.renderer.get_frame()

        # Scale sigma with render resolution so blur looks consistent
        current_height = screen_pixels.shape[0]
        dynamic_sigma = sigma * (current_height / 480.0)

        # Do not blur across color channels (last sigma = 0)
        blurred_pixels = gaussian_filter(
            screen_pixels.astype(float),
            sigma=(dynamic_sigma, dynamic_sigma, 0),
        )
        blurred_pixels = np.clip(blurred_pixels, 0, 255).astype(np.uint8)

        super().__init__(blurred_pixels, **kwargs)

        # Frame geometry: MovingCamera has .frame; normal Camera uses attrs
        cam = scene.camera
        if hasattr(cam, "frame"):
            frame_w = cam.frame.get_width()
            frame_h = cam.frame.get_height()
            frame_center = cam.frame.get_center()
        else:
            frame_w = cam.frame_width
            frame_h = cam.frame_height
            frame_center = np.array(cam.frame_center, dtype=float)

        self.set_width(frame_w)
        self.set_height(frame_h)
...
class TrueGaussianBlur2(ImageMobject):
    def __init__(
        ...
        **kwargs,
        ...
        """
        Soft Gaussian glow/blur baked from a VMobject into an ImageMobject.

        Parameters
        ----------
        mobject :
            Source shape/text whose stroke/fill color is sampled.
        resolution : int
            Pixel grid size (higher = smoother, slower). Try 128–512.
        sigma : float
            Blur radius in *pixels* of the internal grid.
            Bigger = softer / more spread. Useful range ~3–30.
            Examples: 4 = subtle, 12 = medium, 20 = very soft.
        padding : float
            Extra world-space margin around the mobject bbox so the
            blur is not clipped at the edges.
        intensity : float
            Multiplier for final alpha (blur strength / visibility).
            1.0 = normal, 1.5 = stronger glow, 0.5 = subtle.
        samples_per_curve : int
            How densely each curve is rasterized before blurring.

        Notes
        -----
        This is a curve-sample + Gaussian glow, not a true filled-shape
        raster blur. For full-screen blur use ``ScreenBlur``.
        """
        self.source_mobject = mobject
        ...
        for submob in mobject.get_family():
            if not hasattr(submob, "points") or len(submob.points) == 0:
                continue
            if len(submob.submobjects) > 0:
                continue  # only leaf paths

            color_rgb = color_to_rgb(submob.get_color())
            try:
                fill_op = submob.get_fill_opacity()
            except Exception:
                fill_op = 0
            try:
                stroke_op = submob.get_stroke_opacity()
            except Exception:
                stroke_op = 1.0
            base_alpha = fill_op if fill_op > 0.01 else max(stroke_op, 0.2)

            try:
                dense = submob.copy()
                dense.insert_n_curves(samples_per_curve)
                points = dense.points
            except Exception:
            ...
            a_grid[y_indices, x_indices] = np.maximum(
                a_grid[y_indices, x_indices], base_alpha
            )
        ...
        if blurred_a.max() > 0:
            final_a = (blurred_a / blurred_a.max()) * float(intensity)
        else:
            final_a = blurred_a
